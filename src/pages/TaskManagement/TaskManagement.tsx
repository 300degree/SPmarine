...
export default function TaskManagement({}: Props) {
	...
	return (
		<Card className="h-full w-full">
			<div className="flex justify-between p-4 gap-x-4 items-center">
				<Typography variant="h5" className="text-gray-700 ml-5 mt-auto">
					Task Schedule
				</Typography>
				<div className="flex gap-x-3">
					...
					<div className="flex gap-x-2">
						<button className="flex gap-x-2 items-center h-11 py-1 px-3 bg-red-50 border-red-100 border text-red-300 font-normal text-sm rounded-lg cursor-pointer transition active:scale-95">
							<FaRegTrashAlt />
							Remove
						</button>
				...
				<div className="max-w-full overflow-x-auto">
					<Table>
						...
						<TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
							{data.map((item, index) => (
								<TableBodys item={item} index={index} key={item.id} />
							))}
						</TableBody>
					</Table>
				</div>
